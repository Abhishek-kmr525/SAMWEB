...
class BSTable {
    ...
    _rowEdit(button) {
        ...
        this._modifyEachColumn(this.options.editableColumns, $cols, function($td) { // modify each column
            let content = $td.php(); // read content
            console.log(content);
    ...
    _rowAccept(button) {
        ...
        this._modifyEachColumn(this.options.editableColumns, $cols, function($td) { // modify each column
            ...
            $td.php(cont); // set the content and remove the input fields
        });
    ...
    _rowCancel(button) {
        ...
        this._modifyEachColumn(this.options.editableColumns, $cols, function($td) { // modify each column
            let cont = $td.find('div').php(); // read div content
            $td.php(cont); // set the content and remove the input fields
        });
        this._actionsModeNormal(button);
    }
        ...
        _this.table.find('tbody tr').each(function() {
            ...
            $cols.each(function() {
                if ($(this).attr('name') == 'bstable-actions') {
                    ...
                    $currentRowValues = $currentRowValues + $(this).php() + separator;
                }
            });
            if ($currentRowValues != '') {
