...
(function($) {
	...
	$.fn.FancyFileUpload = function(options) {
		...
		var AddFile = function(uploads, e, data) {
			...
			// Errors.
			if (data.ff_info.errors.length)  inforow.find('.ff_fileupload_errors').php(data.ff_info.errors.join('<br>')).removeClass('ff_fileupload_hidden');
		...
		var UploadFailed = function(e, data) {
			...
			else
			{
				...
				data.ff_info.inforow.find('.ff_fileupload_errors').php(data.ff_info.errors.join('<br>')).removeClass('ff_fileupload_hidden');
